...
# 탐지 결과 박스 색상 (BGR, OpenCV 용) - 빨간색 -> 초록색으로 변경
BOX_COLOR_BGR = (110, 158, 29)  # BRAND_BORDER(#1D9E75)에 대응하는 BGR 값
...
def run_detection(raw_image: Image.Image, model, conf_threshold: float):
    ...
    for box in results.boxes:
        ...
        # 탐지 박스 색상: 초록색(BOX_COLOR_BGR)으로 표시
        cv2.rectangle(
            output_img, (int(x1), int(y1)), (int(x2), int(y2)), BOX_COLOR_BGR, 4
        )
        cv2.putText(
            output_img,
            str(det_id),
...
def render_upload() -> None:
    ...
    if raw_image is not None:
        ...
        st.markdown(
            f'<div class="preview-box"><img src="data:image/png;base64,'
            f'{image_data}" alt="preview"></div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            ...
            <div class="preview-box">
                <div class="preview-placeholder">
                    ...
                    <strong>사진을 선택하면 미리보기가 표시돼요</strong>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
def render_processing() -> None:
    st.markdown(
        ...
        <div class="splash-wrap" style="height:55vh;">
            ...
            <div style="margin-top:18px; font-size:16px; color:#444;">
                YOLO 모델로<br>알약을 인식하고 있어요...
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_result() -> None:
    ...
    if output_rgb is not None:
        st.image(output_rgb, caption="AI 탐지 결과", use_container_width=True)
        ...
        for det in detections:
            ...
            st.markdown(
                ...
                <div class="drug-card">
                    ...
                    <p class="drug-sub">{category} · 신뢰도 {det["conf"] * 100:.1f}%</p>
            ...
            if used_fallback:
                st.caption(
                    ...
                    "부작용·복용법·보관법 등 상세 정보는 없습니다."
                )
                continue

            side_effect = get_field(matched_row, "부작용", default="정보 없음")
            usage = get_field(matched_row, "복용법", "용법용량", default="정보 없음")
            storage = get_field(
                matched_row,
                "보관방법",
                "저장방법",
                default="직사광선을 피하고 실온에서 보관하세요.",
            )

            with st.expander("부작용", expanded=False):
                st.write(side_effect)
            ...
            if "임부금기_해당" in matched_row.index:
                with st.expander("🚨 임부 / 연령 / 병용 금기 정보", expanded=False):
