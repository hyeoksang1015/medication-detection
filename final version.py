<<<<<<< HEAD
"""
헬스잇(HealthEat) - YOLOv8 알약 탐지 데모 (hyeoksang.py, CSV 매핑 버전)
화면 흐름: 스플래시 -> 사진 입력 -> 인식 중 -> 결과 출력
세션 상태(KeyError) 해결 및 CSV 기반 의약품 상세 매핑 시스템 연동 완료.
...
def load_drug_csv_info(path: str):
    """의약품 상세정보 CSV 파일을 안전하게 로드합니다.
    매칭용으로 공백을 제거한 clean_dl_name 컬럼을 추가합니다."""
    if not path or not os.path.exists(path):
        return None
    try:
        df = pd.read_csv(path, encoding="utf-8").fillna("-")

        if "dl_name" not in df.columns:
            st.error(
                f"⚠️ CSV 파일에 'dl_name' 컬럼이 존재하지 않습니다. "
                f"현재 컬럼: {list(df.columns)}"
            )
            return None

        df["clean_dl_name"] = (
            df["dl_name"].astype(str).str.replace(r"\s+", "", regex=True)
        )
        return df
    except Exception as e:
...
def match_drug_row(drug_df, drug_name: str, cls_idx: int):
    """YOLO 예측 클래스명을 CSV의 dl_name과 매칭합니다.
    1) 공백 제거 후 완전 일치
    2) 완전 일치가 없으면 부분 포함 매칭으로 재시도
    3) 그래도 없으면 idx(클래스 인덱스) 컬럼으로 매칭 시도
    """
    if drug_df is None:
        return None

    clean_name = str(drug_name).replace(" ", "").strip()

    exact = drug_df[drug_df["clean_dl_name"] == clean_name]
    if not exact.empty:
        return exact.iloc[0]

    partial = drug_df[
        drug_df["clean_dl_name"].str.contains(clean_name, na=False, regex=False)
    ]
    if not partial.empty:
        return partial.iloc[0]

    if "idx" in drug_df.columns:
        idx_match = drug_df[drug_df["idx"].astype(str) == str(cls_idx)]
        if not idx_match.empty:
            return idx_match.iloc[0]
...
def render_result() -> None:
    ...
    # 세션 상태에서 안전하게 값을 가져오고 없으면 기본값 사용
    drug_info_path = st.session_state.get("drug_info_path", "drug_full_info.csv")
    drug_df = load_drug_csv_info(drug_info_path)
    ...
    if not detections:
        ...
        for det in detections:
            ...
            display_name = (
                matched_row.get("dl_name", pred_name)
                if matched_row is not None
                else pred_name
            )
            company = (
                matched_row.get("제조사", "정보 없음")
                if matched_row is not None
                else "정보 없음"
            )
            category = (
                matched_row.get("효능군명", "정보 없음")
                if matched_row is not None
                else "정보 없음"
            )
            drug_type = (
                matched_row.get("전문일반", "정보 없음")
                if matched_row is not None
                else "정보 없음"
            )
            ...
            if matched_row is None:
                st.caption(
                    f"💡 `{drug_info_path}` CSV 파일에 이 알약 이름"
                    f"(`{pred_name}`)과 일치하는 매핑 정보가 없습니다."
                )
                continue
...
if __name__ == "__main__":
    main()
>>>>>>> ec9f85c3eb99603167381f0960b4408142cfb010
